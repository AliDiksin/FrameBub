...
def write_cache(path: Path, media: list[tuple[str, str, str, str]], notes_cache: dict[str, dict[str, str]]) -> tuple[int, int, int]:
    ...
    path.write_text(
        ...
        f"BBCF_MOVE_IMAGE_URLS = {json.dumps(image_cache, indent=2, sort_keys=True)}\n\n"
        f"BBCF_HITBOX_DATA = {json.dumps(hitbox_cache, indent=2, sort_keys=True)}\n\n"
        f"BBCF_MOVE_NOTES = {json.dumps(notes_cache, indent=2, sort_keys=True)}\n",
        encoding="utf-8",
    )
    return (
