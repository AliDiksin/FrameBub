...
from scripts.scraper_utils import fetch_url, normalize_move_cache_key  # noqa: E402
...
def fetch_html(url: str, sleep_seconds: float = 0.0) -> str:
    return fetch_url(url, sleep_seconds=sleep_seconds, decode_errors="replace")
...
def write_cache(path: Path, images: dict[str, dict[str, str]], notes: dict[str, dict[str, str]]) -> tuple[int, int]:
    write_python_constants(
        path,
        {"COTW_MOVE_IMAGE_URLS": images, "COTW_MOVE_NOTES": notes},
        "scrape_cotw_dreamcancel.py",
    )
